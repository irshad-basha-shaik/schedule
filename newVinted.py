...
Header={'Cookie':'csrftoken=bD0kdBbkK2ufeX55O4XtK4AF6v2k6QD0; JSESSIONID=1E6DFA4C57FB2EC252698827A6B1EA01'}
...
'''
Cookie: csrftoken=bD0kdBbkK2ufeX55O4XtK4AF6v2k6QD0; JSESSIONID=1E6DFA4C57FB2EC252698827A6B1EA01
...
def findImagePath():
    start1 = gui.locateCenterOnScreen("imagePath3.PNG")
    if (start1 != None):
        gui.moveTo(start1)
        gui.click()
        return None
...
def locateImage():
    start1 = gui.locateCenterOnScreen("imageName4.PNG")
    if (start1 != None):
        gui.moveTo(start1)
        gui.click()
        return None
    else:
        start1 = gui.locateCenterOnScreen("imageName2.PNG")
        if (start1 != None):
            gui.moveTo(start1)
            gui.click()
            return None
...
def deleteImage(a):
    try:
        os.remove('C:\\Users\\yaseen\\Desktop\\irshad\\schedule\\'+a+".png")
    except:
        pass
    return None
...
def findImage(a,obj):
    findImageUpload()
    locateImage()
    gui.typewrite('C:\\Users\\yaseen\\Desktop\\irshad\\schedule\\'+a+".png", interval=0.05)
...
def findCategories(obj):
    ...
    findCat()
    inspectOpen()
    #gui.typewrite("function doIt(obj1){a =document.getElementsByClassName('input-dropdown__content')[0].children[0].children[0].children; console.log(a); obj2 = obj1.split('/'); for(var j=0;j<obj2.length;j++){ obj=obj2[j]; for(var i=0;i<a.length;i++){ console.log(obj+'=='+a[i].textContent); if(obj==a[i].textContent){  a[i].children[0].click()}}}}",interval=0.03)
...
def findWriteConsole():
    ...
    if (start != None):
        ...
        start = gui.locateCenterOnScreen("consoleWrite2.PNG")
        if (start != None):
            gui.moveTo(start)
            gui.click()
            return None
...
def findSize(obj):
    ...
    if (start != None):
        ...
        if (start != None):
            ...
            findSize(obj)
    findWriteConsole()
    #gui.typewrite("function doItC(obj1){var c = document.getElementsByClassName('input-dropdown__content')[0].children[0].children[2].children; for (var i = 0; i < c.length; i++) {console.log(obj+'=='+c[i].textContent); if (obj1 == c[i].textContent) {c[i].children[0].click();}}}",interval=0.02)
...
def findCondition(obj):
    ...
    if (start != None):
        ...
        findCondition(obj)
    findWriteConsole()
    gui.typewrite("function doItD(obj1){var d = document.getElementsByClassName('input-dropdown__content')[0].children[0].children[0].children; for (var i = 0; i < d.length; i++) {console.log(obj + '==' + d[i].textContent); if (obj1 == d[i].textContent) {d[i].children[0].click();}}}",interval=0.02)
    #gui.typewrite("function doIt(obj1){a =document.getElementsByClassName('input-dropdown__content')[0].children[0].children[0].children;\n console.log(a);\n for(var i=0;i<a.length;i++){\n if(obj1==a[i].textContent){ \n a[i].children[0].click()}}}",interval=0.02)
    #gui.typewrite("function doIt(obj1){a =document.getElementsByClassName('input-dropdown__content')[0].children[0].children[1].children;\n console.log(a);\n obj2 = obj1.split('/');\n for(var j=0;j<obj2.length;j++){\n obj=obj2[j];\n for(var i=0;i<a.length;i++){\n console.log(obj+'=='+a[i].textContent); \n if(obj==a[i].textContent){ \n a[i].children[0].click()}}}}",interval=0.02)
    #gui.press('enter')
    gui.typewrite("doItD('" + obj + "')")
    gui.press('enter')
...
def findSave():
    ...
    if(start!=None):
        ...
        if (start != None):
            ...
            findSave()
def findSaveDraft():
    start = gui.locateCenterOnScreen("saveDraft.PNG")
    if(start!=None):
        gui.moveTo(start)
        gui.click()
        return None
...
def domainUrl():
    ...
    if (start != None):
        ...
        gui.click()
        #k = json.dumps(obj)
        gui.typewrite("https://www.vinted.co.uk/items/new?action=1",interval=0.02)
        gui.press('enter')
        time.sleep(1)
        return None
    else:
...
def domainUrl1(obj):
    ...
    gui.click(285,55)
    k = json.dumps(obj)
    gui.typewrite("https://www.vinted.co.uk/items/new?action=1&action2="+k)
    gui.press('enter')
    time.sleep(1)
    return None
def skewImage(obj):
...
def startChrome1(obj):
    k = json.dumps(obj)
    os.system('cmd /k "C:\\Users\\yaseen\\Desktop\\pyAuto.lnk https://www.vinted.co.uk/items/new?action=1;action2="'+ k)
    gui.press('enter')
    time.sleep(1)
    return None
def startChrome():
    ...
    gui.click()
    gui.typewrite('Chrome', interval=0.02)
    gui.press('enter')
    time.sleep(1)
    gui.hotkey('win', 'up')
    gui.keyDown('alt')
    ...
    return None
def typeData(obj):
    start = gui.locateCenterOnScreen("typeData.PNG")
    if (start != None):
        #gui.mouseInfo()
        gui.moveTo(start)
        gui.click()
        ...
        typeData(obj)
#obj1 = {'id': '402880848976ba5e018976bc31c40000', 'itemId': '402880848561498c01856180aa1a00c4', 'url': 'https://i.ibb.co/fd1TSXr/Screenshot-from-2023-07-15-10-37-55.png', 'category': 'Bags', 'status': 'not sent', 'unisex': 'true', 'imageUrl': 'https://i.ibb.co/PxZK3qK/f.png', 'imageUrls': None, 'description': 'https://i.ibb.co/fd1TSXr/Screenshot-from-2023-07-15-10-37-55.png', 'platform': None, 'isbn': None, 'brand': 'Topshop', 'mpn': None, 'color': None, 'size': None, 'parcelSize': 'medium', 'price': 12, 'quantity': 0, 'title': 'fghij', 'ean': 'https://i.ibb.co/fd1TSXr/Screenshot-from-2023-07-15-10-37-55.png', 'conditionId': 'New without tagsA brand-new, unused item without tags or original packaging.', 'sku': 'https://i.ibb.co/fd1TSXr/Screenshot-from-2023-07-15-10-37-55.png', 'rating': None, 'material': None, 'measurements': None, 'ownerId': None, 'tooltip': '/Women/Bags/Satchels', 'accountId': '402880848561498c01856180aa1a00c4', 'createdAt': None, 'originalPriceNumeric': 0.0, 'itemClosingAction': None, 'modifiedDate': None}
def postVinted(obj):
    obj["title"]=obj["title"]+"$"
    image_id=uuid.uuid4()
    uploadImage(str(image_id),obj["imageUrl"])
    # gui.hotkey("ctrl", "t")
    time.sleep(1)
    startChrome(obj)
    # domainUrl1(obj)
    time.sleep(1)
    findImage(str(image_id),obj["imageUrl"])
    time.sleep(1)
    gui.press("enter")
    findTitle(obj["title"])
    time.sleep(5)
